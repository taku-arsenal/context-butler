...
本アプリは AI-DLC の Unit 分解として、以下 4 つの専門 Agent で構成されます。
...
flowchart LR
  ...
  MSG --> SLACK[Slack Interactivity Request]
  SLACK --> APIGW[API Gateway]
  APIGW --> ACK[Lambda: Ack\n署名検証・job作成\n3秒以内に200 OK]
  ACK --> SQS[SQS FIFO\n非同期化・重複排除]
  SQS --> WORKER[Lambda Worker\nSlack API・Agent起動]

  WORKER --> AGENT[AgentCore Runtime\nStrands Orchestrator]

  AGENT --> A1[Unit 1\n省略抽出 Agent]
  A1 --> A2[Unit 2\n文脈取得 Agent]
  A2 --> THREAD[Slack Thread API]
