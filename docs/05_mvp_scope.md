...
| M-06 | Unit 1: 省略抽出 | 省略点・必要文脈を構造化 JSON で抽出する |
| M-07 | Unit 2: 文脈取得（最小版） | Slack スレッドとデモ用 KB から必要文脈を取得する |
| M-08 | Unit 3: 補足文生成 | 背景・前提・次アクションを整理した補足文を生成する |
| M-09 | Unit 4: リテラシーレビュー | 品質・安全性を確認し最終文を出力する |
| M-10 | AgentCore Runtime + Strands Orchestrator | MVP から 4 Unit を順次制御する第一候補として利用する |
...
| # | 機能 | 説明 |
|---|------|------|
| S-01 | Unit 2: 文脈取得（強化版） | チャンネル要約・Drive・GitHub からの文脈取得を追加する |
| S-02 | Bedrock Knowledge Bases の改善 | デモ用 KB の検索精度を高める |
...
タスク:
  [ ] Bedrock 呼び出し実装
  [ ] 省略抽出プロンプト実装（Unit 1 簡易版）
  [ ] 文脈取得プロンプト実装（Unit 2 最小版: Slack スレッド + デモ用 KB）
  [ ] 補足文生成プロンプト実装（Unit 3）
  [ ] リテラシーレビュープロンプト実装（Unit 4）
...
```
目標: Strands Orchestrator で 4 Unit を順次実行できる

タスク:
  [ ] Unit 1 省略抽出 Agent 実装
  [ ] Unit 2 文脈取得 Agent 実装（Slack スレッド + デモ用 KB）
  [ ] Unit 3 補足文生成 Agent 実装
  [ ] Unit 4 リテラシーレビュー Agent 実装
...
タスク:
  [ ] デモ用 Slack 投稿作成（demo/slack_messages.md）
  [ ] デモ用 Drive 資料作成（demo/drive_docs/）
  [ ] デモ用 GitHub Issue 作成（demo/github_issues.md）
  [ ] デモ用 KB 資料作成（demo/kb_docs/）
  [ ] プレゼン資料作成
  [ ] README 最終整備
  [ ] 構成図最終整備
  [ ] 本番デモ手順作成（docs/10_demo_scenario.md）
```
...
高インパクト・低コスト（最優先）:
  ...
  - 3 秒以内 ack
  - Unit 2 最小文脈取得（Slack スレッド + デモ用 KB）
  - 補足文生成（Unit 3）
  - スレッド返信
...
4. Slack スレッドとデモ用 KB を使った文脈取得が動く
5. AgentCore Runtime + Strands Orchestrator で 4 Unit の処理ログを見せられる
...
| AgentCore Runtime の設定が複雑 | MVP から利用することを第一候補にする。予選直前に詰まる場合のみ、4 Unit 構造を保った Bedrock 直接呼び出しでデモを継続する |
| MCP 接続が複雑 | MVP ではモックで代替 |
| デモ中に生成が遅い | 「生成中です」メッセージで UX を補完 |
| デモ用 KB が不十分 | デモ用 Markdown 資料を事前に充実させる |
