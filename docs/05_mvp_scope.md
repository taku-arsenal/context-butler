...
### MVP Target / Future（実装を目指し、難しい場合は将来対応）

| # | 機能 | 説明 |
|---|------|------|
| C-01 | Google Drive MCP | Drive から議事録・仕様書を取得する。難しい場合はFutureへ回す |
| C-02 | GitHub MCP | GitHub から Issue・PR・README を取得する。難しい場合はFutureへ回す |
...
タスク:
  ...
  [ ] KB 検索の実装
  [ ] Drive MCP 実装（MVP Target / Future）
  [ ] GitHub MCP 実装（MVP Target / Future）
```
...
1. 上記に加えて
2. AgentCore Runtime + Strands Orchestrator で 4 Agent ステージの処理ログを見せられる
3. Slack モーダルで想定読者を選択できる
4. Drive・GitHub からの文脈取得を見せられる
5. フィードバックボタンが機能する
